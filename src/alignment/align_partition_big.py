...
if TEST:
  ...
  in_path = os.path.join( "/data", "project", "SparkMethyl", "test", "SRR306438", "testset" )
  # out_path = os.path.join( "/data", "project", "SparkMethyl", "test", "SRR306438", "testset", "result" )
  out_path = os.path.join( "/data", "project", "SparkMethyl", "test", "SRR306438", "testset", "pyspark_result" )

else:
  ref_path = os.path.join( "/data", "project", "SparkMethyl", "test", "ref_genomes" )
  in_path = os.path.join( "/data", "project", "SparkMethyl", "test", "SRR306438", "realset" )
  out_path = os.path.join( "/data", "project", "SparkMethyl", "test", "SRR306438", "realset", "result" )
...
def align(sc, argv):
  if TEST:
    # ref_file = os.path.join( ref_path, "genome.fa")
    i_file = os.path.join( in_path, "10000.fa")
  else:
    i_file = os.path.join( in_path, "sample.filtered.fa")
  ...
  ### transform input
  c2t_seqs = in_seqs.mapValues( lambda x: x.translate( make_trans_with("W", "C", "T")) )\
    .persist(StorageLevel.MEMORY_AND_DISK)

  g2a_seqs = in_seqs.mapValues( lambda x: x.translate( make_trans_with("W", "G", "A")) )\
    .persist(StorageLevel.MEMORY_AND_DISK)
  ...
  combined_seqs.saveAsTextFile( "file://"+os.path.join( out_path, "combined") )
  ...
  joined_seqs.saveAsTextFile( "file://"+os.path.join( out_path, "joined") )
  ...
  chrkey_rdd.saveAsTextFile( "file://"+os.path.join( out_path, "chrkey") )
  ...
  res_rdd.map( lambda x: res_to_string(x))\
    .saveAsTextFile( "file://"+os.path.join( out_path, "toFile") )
